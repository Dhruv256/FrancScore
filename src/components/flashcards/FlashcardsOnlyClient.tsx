...
export function FlashcardsOnlyClient() {
  ...
  useEffect(() => {
    ...
    const query = new URLSearchParams(reviewIds.length ? { ids: reviewIds.join(","), limit: String(Math.min(reviewIds.length, 50)) } : { limit: "20", level, category });
  ...
  }, [level, category, reviewDeck, progress.cards]);

...
export function FlashcardsOnlyClient() {
  ...
  useEffect(() => {
    setIsLoading(true);
    const query = new URLSearchParams({ limit: "20", level, category });
  ...
  function review(rating: Rating) {
    ...
    setProgress(nextProgress); saveGuestProgress(nextProgress); setFlipped(false); setDrag({ x: 0, y: 0 }); setIndex((value) => (value + 1) % Math.max(cards.length, 1)); announce(rating === "Skip" ? "Card skipped" : `Marked ${rating}`); window.setTimeout(() => { lock.current = false; }, 320);
  }
  ...
  return <div className="motion-enter space-y-6">
    ...
    {!started ? <>
      ...
      <div className="flex items-center justify-between gap-3"><button className="btn btn-secondary" onClick={() => setStarted(false)}>← Decks</button><p className="theme-soft text-sm font-bold">Daily Review · {cards.length ? `${index + 1} / ${cards.length}` : "Loading"}</p><button className="btn btn-ghost" onClick={() => setCoach(true)} aria-label="View gesture guide"><Info className="h-5 w-5" /></button></div>
    ...
    {coach ? <div className="modal-scrim"><motion.div initial={{ opacity: 0, y: 18, scale: .98 }} animate={{ opacity: 1, y: 0, scale: 1 }} className="ui-surface modal-sheet"><p className="theme-soft text-xs font-black uppercase tracking-[.18em]">Gesture guide</p><h2 className="editorial-title mt-2 text-3xl">Learn by feel.</h2><p className="theme-muted mt-4 text-sm leading-7">Tap to flip · swipe left Again · down-left Hard · right Good · up-right Easy · down Skip · hold for Weak · double tap for Mastered.</p><div className="mt-6 flex gap-2"><button className="btn btn-primary" onClick={dismissCoach}>Got it</button><button className="btn btn-secondary" onClick={() => setCoach(false)}>Close</button></div></motion.div></div> : null}
  </div>;
}

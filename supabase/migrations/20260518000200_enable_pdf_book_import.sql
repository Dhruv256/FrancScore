...
alter table public.pdf_import_batches add column if not exists title text;
alter table public.pdf_import_batches add column if not exists chapters_detected integer not null default 0;
alter table public.pdf_import_batches add column if not exists updated_at timestamptz not null default now();

create index if not exists book_sources_active_created_idx on public.book_sources(is_active, created_at desc);
create index if not exists book_chapters_source_order_idx on public.book_chapters(book_source_id, order_index);
create index if not exists book_pages_source_page_idx on public.book_pages(book_source_id, page_number);
create index if not exists book_chunks_source_chapter_idx on public.book_chunks(book_source_id, chapter_id, chunk_index);
create index if not exists book_notes_chapter_type_idx on public.book_notes(chapter_id, note_type);
create index if not exists book_generated_items_chapter_type_idx on public.book_generated_items(chapter_id, item_type);

drop trigger if exists set_book_sources_updated_at on public.book_sources;
create trigger set_book_sources_updated_at before update on public.book_sources
for each row execute function public.set_updated_at();

drop trigger if exists set_book_chapters_updated_at on public.book_chapters;
create trigger set_book_chapters_updated_at before update on public.book_chapters
for each row execute function public.set_updated_at();

drop trigger if exists set_book_notes_updated_at on public.book_notes;
create trigger set_book_notes_updated_at before update on public.book_notes
for each row execute function public.set_updated_at();

drop trigger if exists set_user_book_progress_updated_at on public.user_book_progress;
create trigger set_user_book_progress_updated_at before update on public.user_book_progress
for each row execute function public.set_updated_at();
...
begin
  foreach table_name in array array[
    'book_sources','book_chapters','book_pages','book_chunks','book_notes',
    'book_generated_items','book_import_reports'
  ]
  loop
  ...
  if not exists (select 1 from pg_policies where schemaname = 'public' and tablename = 'book_sources' and policyname = 'Authenticated users can read active internal book sources') then
    create policy "Authenticated users can read active internal book sources"
      on public.book_sources for select to authenticated
  ...
  if not exists (select 1 from pg_policies where schemaname = 'public' and tablename = 'book_chapters' and policyname = 'Authenticated users can read book chapters') then
    create policy "Authenticated users can read book chapters"
      on public.book_chapters for select to authenticated
      using (exists (select 1 from public.book_sources s where s.id = book_source_id and s.is_active = true and s.is_internal = true));
  end if;
  ...
  if not exists (select 1 from pg_policies where schemaname = 'public' and tablename = 'book_chunks' and policyname = 'Authenticated users can read book chunks') then
    create policy "Authenticated users can read book chunks"
      on public.book_chunks for select to authenticated
      using (exists (select 1 from public.book_sources s where s.id = book_source_id and s.is_active = true and s.is_internal = true));
  end if;
  ...
  if not exists (select 1 from pg_policies where schemaname = 'public' and tablename = 'book_generated_items' and policyname = 'Authenticated users can read book generated items') then
    create policy "Authenticated users can read book generated items"
      on public.book_generated_items for select to authenticated
      using (exists (select 1 from public.book_sources s where s.id = book_source_id and s.is_active = true and s.is_internal = true));
  end if;
  ...
  if not exists (select 1 from pg_policies where schemaname = 'public' and tablename = 'user_book_progress' and policyname = 'Users can manage their own book progress') then
    create policy "Users can manage their own book progress"
      on public.user_book_progress for all to authenticated
  ...
  if not exists (select 1 from pg_policies where schemaname = 'public' and tablename = 'user_book_answers' and policyname = 'Users can manage their own book answers') then
    create policy "Users can manage their own book answers"
      on public.user_book_answers for all to authenticated
      using (user_id = auth.uid())
      with check (user_id = auth.uid());
  end if;
end $$;
